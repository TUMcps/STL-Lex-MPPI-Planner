...
CoordinateSystemWrapper::CoordinateSystemWrapper(Eigen::Ref<RowMatrixXd> refPath)
    : m_refPath(refPath), m_refPolyline(util::matrixToVector2d(m_refPath)), m_system(std::make_shared<geometry::CurvilinearCoordinateSystem>(m_refPolyline))
{
...
Eigen::VectorXd CoordinateSystemWrapper::computeCurvatureFromPolyline(geometry::EigenPolyline &polyline)
{
    ...
    std::vector<double> curvatures_vec = m_system->curvatureVector();
    // Convert std::vector to Eigen::VectorXd
    Eigen::VectorXd curvatures = Eigen::Map<Eigen::VectorXd>(curvatures_vec.data(), curvatures_vec.size());
    return curvatures;
}
