...
/**
 * @brief Fill coordinates for a trajectory sample, computing cartesian coordinates
 *        from curvilinear trajectory polynomials.
 *
 * @param trajectory The trajectory sample to fill coordinates for
 * @param lowVelocityMode Whether to use low velocity mode for coordinate transformation
 * @param initialOrientation Initial orientation for the trajectory
 * @param coordinateSystem Coordinate system wrapper for transformations
 * @param horizon Planning horizon
 */
inline void fillTrajectoryCoordinates(TrajectorySample &trajectory,
                                      ...
                                      double horizon)
{
    ...
    const auto &ccs = coordinateSystem->getSystem();
    ...
    {
        length = static_cast<size_t>(1 + (horizon / trajectory.m_dT));
    }
    else
    {
        length = actualLength;
    ...
    for (int iii = 0; iii < length; ++iii)
    {
        ...
        auto s_idx_opt = coordinateSystem->getS_idx(trajectory.m_curvilinearSample.s[iii]);
        ...
        double sLambda = coordinateSystem->getSLambda(trajectory.m_curvilinearSample.s[iii], s_idx);
        ...
        else
        {
            ...
            {
                trajectory.m_cartesianSample.theta[iii] = (iii == 0) ? initialOrientation : trajectory.m_cartesianSample.theta[iii - 1];
        ...
        trajectory.m_cartesianSample.kappa[iii] = (dpp + (k_r * dp + k_r_d * trajectory.m_curvilinearSample.d[iii]) * tanTheta) * cosTheta * std::pow(cosTheta / oneKrD, 2) + (cosTheta / oneKrD) * k_r;
        ...
        trajectory.m_cartesianSample.acceleration[iii] = trajectory.m_curvilinearSample.sss[iii] * oneKrD / cosTheta +
                                                         std::pow(trajectory.m_curvilinearSample.ss[iii], 2) / cosTheta * (oneKrD * tanTheta * (trajectory.m_cartesianSample.kappa[iii] * oneKrD / cosTheta - k_r) - (k_r_d * trajectory.m_curvilinearSample.d[iii] + k_r * dp));
