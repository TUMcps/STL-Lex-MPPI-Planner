...
TrajectorySample::TrajectorySample(double dT,
    ...
    : m_dT(dT), m_harm_occ_module(0), m_uniqueId(uniqueId), m_samplingParameters(samplingParameters), m_trajectoryLongitudinal(std::make_shared<TrajectorySample::LongitudinalTrajectory>(trajectoryLongitudinal)), m_trajectoryLateral(std::make_shared<TrajectorySample::LateralTrajectory>(trajectoryLateral))
{
}
...
TrajectorySample::TrajectorySample(double dT,
                                   ...
                                   int uniqueId,
                                   Eigen::VectorXd samplingParameters)
    : m_dT(dT), m_harm_occ_module(0), m_uniqueId(uniqueId), m_samplingParameters(samplingParameters), m_trajectoryLongitudinal(std::make_shared<TrajectorySample::FixedLongitudinalTrajectory>(trajectoryLongitudinal)), m_trajectoryLateral(std::make_shared<TrajectorySample::LateralTrajectory>(trajectoryLateral))
{
}
    ...
    bool lowVelocityMode)
{
    ...
    double dpp = -(k_r_d * x_cl.x0_lat[0] + k_r * dp) * std::tan(theta) + ((1 - k_r * x_cl.x0_lat[0]) / std::pow(std::cos(theta), 2)) * (kappa * (1 - k_r * x_cl.x0_lat[0]) / std::cos(theta) - k_r);
    ...
    x_cl.x0_lon[2] = x_0.acceleration - std::pow(x_cl.x0_lon[1], 2) / std::cos(theta) * ((1 - k_r * x_cl.x0_lat[0]) * std::tan(theta) * (kappa * (1 - k_r * x_cl.x0_lat[0]) / std::cos(theta) - k_r) - (k_r_d * x_cl.x0_lat[0] + k_r * dp)) / ((1 - k_r * x_cl.x0_lat[0]) / std::cos(theta));
