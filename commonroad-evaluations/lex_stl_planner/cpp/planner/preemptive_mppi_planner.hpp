...
// Preemptive lexicographic MPPI planner implementation.
class PreemptiveMPPIPlanner : public BasePlanner
{
public:
    ...
    // Constructor.
    explicit PreemptiveMPPIPlanner(const Config &cfg,
...
private:
    ...
    mppiStep(Eigen::MatrixXd control_input,
             int iteration,
             int objective_level,
             const std::vector<double> &hard_constraints);

    double calculateInputCost(const Eigen::MatrixXd &eps,
                              const std::vector<Eigen::RowVectorXd> &weights) const;

    double calculateRemainingInputCost(const Eigen::MatrixXd &u) const;

    bool satisfiesHardConstraints(const std::vector<double> &rule_costs,
